...
*/
function conc(a, b) {
  let firstChar = a.slice(0, 2);
  let lastChar = b.slice(- 3);
  let stringConc = firstChar + lastChar;
  let stringMaiusc = stringConc.toUpperCase();
  console.log(stringMaiusc);
}
conc("black", "sabbath");
...
*/
let sumNumbers = 0;
filteredNumbers.forEach(item => sumNumbers += item);
console.log(sumNumbers);

/* ESERCIZIO 5 (reduce)
  Scrivi una funzione per sommare i numeri contenuti in un array
*/
let sumNumbers2 = filteredNumbers.reduce((tot,value) => tot+value,0);
...
*/
function func(n) {
  let incrNumbers = filteredNumbers.map(item => item+n)
  console.log(incrNumbers);
}
func(4)
...
*/
let string = ['hello','darkness','my','old','friend'];
function strings() {
  let stringLength = string.map((item) => item.length);
